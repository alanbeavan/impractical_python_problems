...
def check_lines(fake, real):
    """Check if the number of blank lines is enough to hold the message."""
    length_real = len(real)
    empty_count = 0
    for line in fake:
        if line == "":
            empty_count += 1
    if empty_count >= length_real:
        return 1
    return 0
...
def main():
    ...
    if check_lines(fake_list, real_list):
        interleave_message(blank_doc, fake_list, real_list)
        print("Done!")
    else:
        print("Message too long for the number of blank lines in the fake" \
              "message")
