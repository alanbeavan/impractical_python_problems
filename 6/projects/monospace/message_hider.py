...
I couldn't get it not to do funny line breaks, so it requires
paragraphs of one line or fewer in the fake message.
"""
...
def load_assets(blank_filename, fake_filename, real_filename):
    ...
    for style in ["Normal", "Heading 1", "Title"]:
        blank_doc.styles[style].font.name = "Courier"
    ...
    real = "".join(real_paragraphs)

    return blank_doc, fake_paragraphs, real
...
def interleave_message(doc, fake, real):
    ...
    for line in fake:
        ...
        copy = list(line)
        if count_real < length_real:
            start = 0
            for i in range(len(line)):
                if count_real < length_real:
                    if line[i] == " ":
                        paragraph.add_run("".join(copy[start:i]))
                        #change_colour(255, 0, 0)
                        paragraph.add_run(real[count_real]).font.color.rgb = RGBColor(255, 255, 255)
                        #change_colour(0, 0, 0)
                        count_real += 1
                        start = i+1
                else:
                    paragraph.add_run("".join(copy[start:]))
                    break
            #paragraph = doc.add_paragraph(real[count_real])
            #paragraph_index = len(doc.paragraphs) - 1
            #run = doc.paragraphs[paragraph_index].runs[0]
            #font = run.font
            #font.color.rgb = RGBColor(255, 0, 0) #Currently red
            #count_real += 1
        else:
            paragraph.add_run("".join(copy))
...
def main():
    """Do the things."""
    blank_doc, fake_list, real = load_assets("template.docx",
                                             "fakeMessage.docx",
                                             "realMessage.docx")
    add_letterhead(blank_doc)
    interleave_message(blank_doc, fake_list, real)
    print("Done!")
